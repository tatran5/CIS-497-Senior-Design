...
public class FractalTerrain : MonoBehaviour
{
    ...
    public float m_heightRange = 0.3f;
    int m_res;
    Mesh mesh;
    Vector3[] vertices;
    int[] triangles;
    ...
    void DiamondStep(int z, int x, int sideLength, float reduction)
    {
        ...
        /* Handle top edge (and similarly for left edge): 
        ...
        int centerIdxTopSquare = centerIdx - m_res * sideLength;
        ...
        int centerIdxLefSquare = midLefIdx - halfSideLength;
        ...
        if (isRigSquare) HandleMidpoint(true, -1, centerIdx + halfSideLength, centerIdx, topRigIdx, botRigIdx, normal, reduction);
        if (isBotSquare) HandleMidpoint(true, -1, botLefIdx + halfSideLength, centerIdx, botLefIdx, botRigIdx, normal, reduction);
    }
        ...
        int centerIdx, int endpoint0Idx, int endpoint1Idx, Vector3 normal, float reduction)
    {
        ...
        vertices[midpointIdx] = isEdgeCase ?
            Global.Average2Vector(vertices[endpoint0Idx], vertices[endpoint1Idx]) :
